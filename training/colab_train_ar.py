"""
Arabic CST vs SentencePiece BPE — Fair comparison (Google Colab)

Upload these 8 files to Colab:
  - cst-ar-8k-train-100000.jsonl  + cst-ar-8k-train-100000-vocab.json
  - spm-ar-8k-train-100000.jsonl  + spm-ar-8k-train-100000-vocab.json
  - cst-ar-32k-train-100000.jsonl + cst-ar-32k-train-100000-vocab.json
  - spm-ar-32k-train-100000.jsonl + spm-ar-32k-train-100000-vocab.json

Same architecture, same vocab budget → fair test.
...
EXPERIMENTS = [
    {
        "group": "Arabic 8K vocab",
        "pairs": [
            {"name": "AR-CST-8K",  "data": "cst-ar-8k-train-100000.jsonl",  "vocab": "cst-ar-8k-train-100000-vocab.json"},
            {"name": "AR-SPM-8K",  "data": "spm-ar-8k-train-100000.jsonl",  "vocab": "spm-ar-8k-train-100000-vocab.json"},
        ],
    },
    {
        "group": "Arabic 32K vocab",
        "pairs": [
            {"name": "AR-CST-32K", "data": "cst-ar-32k-train-100000.jsonl", "vocab": "cst-ar-32k-train-100000-vocab.json"},
            {"name": "AR-SPM-32K", "data": "spm-ar-32k-train-100000.jsonl", "vocab": "spm-ar-32k-train-100000-vocab.json"},
        ],
    },
]
...
print("\n" + "=" * 80)
print("  ARABIC FAIR COMPARISON: CST vs SentencePiece (same vocab budget)")
print("=" * 80)
...
for exp in EXPERIMENTS:
    ...
    if len(group_results) == 2:
        a, b = group_results
        bpc_diff = ((b["best_val_bpc"] - a["best_val_bpc"]) / b["best_val_bpc"]) * 100
        print(f"    {a['name']}: BPC = {a['best_val_bpc']:.4f}    {b['name']}: BPC = {b['best_val_bpc']:.4f}    → {bpc_diff:.1f}% reduction")
