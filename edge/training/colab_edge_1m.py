"""
Arabic CST Edge Model — 1M Training + ONNX Export (Google Colab)

Prerequisites:
  1. Run tokenize_1m.py first to produce train-1000000.jsonl + vocab
  2. Or upload pre-tokenized files to /content/cst_1m/

Run:
  !python colab_edge_1m.py

Config changes from 100K:
  - 5 epochs (was 3)
  - Same 6.8M param model (n_embd=256, n_layer=6, n_head=4)
  - vocab_size fits actual data (auto-detected)
"""
...
DATA_FILE = "train-1000000.jsonl"
VOCAB_FILE = "train-1000000-vocab.json"
OUT_DIR = "/content/edge_model_1m"
...
LR = 3e-4
MAX_LEN = 128
VAL_RATIO = 0.05   # 5% val for 1M (50K val is plenty)
...
def train_model(train_ids, train_chars, val_ids, val_chars, vocab_size):
    device = "cuda" if torch.cuda.is_available() else "cpu"

    print(f"\n{'='*60}")
    print(f"  AR-CST-8K Edge — 1M Training")
...
def build_vocab_json(vocab_path, out_dir):
    ...
    # vocab from tokenize_1m.py is {token: id} dict
    if isinstance(v, dict) and all(isinstance(val, int) for val in v.values()):
        tok2id = v
        id2tok = {str(vid): tok for tok, vid in v.items()}
        size = max(v.values()) + 1
    elif isinstance(v, list):
        tok2id = {e["token"]: int(e["id"]) for e in v}
        id2tok = {str(e["id"]): e["token"] for e in v}
        size = max(int(e["id"]) for e in v) + 1
    else:
        raise ValueError(f"Unknown vocab format: {type(v)}")
...
def main():
    ...
    if not os.path.exists(data_path):
        ...
        print(f"Run tokenize_1m.py first, or upload {DATA_FILE} to {DATA_DIR}/")
        return
    ...
    with open(vocab_path) as f:
        vocab = json.load(f)
    if isinstance(vocab, dict) and all(isinstance(val, int) for val in vocab.values()):
        vocab_size = max(vocab.values()) + 1
    elif isinstance(vocab, list):
        vocab_size = max(int(e["id"]) for e in vocab) + 1
    else:
    ...
    print(f"  ONNX fp32:  {fp32_mb:.1f} MB")
    print(f"  ONNX int8:  {int8_mb:.1f} MB")
